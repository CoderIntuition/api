...
public interface ProblemRepository extends JpaRepository<Problem, Long> {
    @Query("SELECT p FROM Problem p WHERE p.category = (:category) AND (p.deleted IS NULL OR p.deleted = 0) ORDER BY p.id ASC")
    Page<Problem> findByCategory(@Param("category") Category category, Pageable pageable);

    @Query("SELECT p FROM Problem p WHERE p.urlName = :urlName AND (p.deleted IS NULL OR p.deleted = 0)")
    Optional<Problem> findByUrlName(@Param("urlName") String urlName);

    @Query("SELECT p FROM Problem p WHERE p.id = :id AND (p.deleted IS NULL OR p.deleted = 0)")
